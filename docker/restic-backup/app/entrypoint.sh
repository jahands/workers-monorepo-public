...
restic snapshots -c --latest=1 --host="$(hostname)" ${RESTIC_INIT_ARGS} &>/dev/null
status=$?
...
echo "${BACKUP_CRON} /usr/bin/flock -n /var/run/backup.lock /usr/local/bin/backup >> /var/log/cron.log 2>&1" > /var/spool/cron/crontabs/root
...
if [ -n "${CHECK_CRON}" ]; then
	...
	echo "${CHECK_CRON} /usr/bin/flock -n /var/run/backup.lock /usr/local/bin/check >> /var/log/cron.log 2>&1" >> /var/spool/cron/crontabs/root
fi
